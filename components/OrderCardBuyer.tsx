...
import { CurrentOrderDto } from 'types'

export const OrderCardBuyer = ({ order }: { order: CurrentOrderDto }) => {
  return (
    <div className="w-full border rounded-md border-inherit p-4">
      <div className="flex">
        <div className="flex-1">
          <div className="text-md font-bold">{order.restaurants.name}</div>
          <div className="mt-1 mb-3 whitespace-pre-line">{order.order_text}</div>
          <div>{new Date(order.ordered_at).toLocaleString([], {day: 'numeric', month: 'short', hour:'2-digit', minute:'2-digit'})}</div>
        </div>
